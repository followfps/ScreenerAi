...
func (c *QwenClient) SendMessageV2(ctx context.Context, message any, model, chatID, parentID string, tools any, toolChoice any, systemMessage any) (SendMessageResult, error) {
	if strings.TrimSpace(model) == "" {
		model = "qwen-max-latest"
	}
	...
	for attempt := 0; attempt < 10; attempt++ {
		...
		comp, status, body, err := c.sendMessageOnce(ctx, token.Token, message, model, chatID, parentID, tools, toolChoice, systemMessage)
		if err == nil {
			if token.ID != "" {
...
func (c *QwenClient) sendMessageOnce(ctx context.Context, bearerToken string, message any, model, chatID, parentID string, tools any, toolChoice any, systemMessage any) (map[string]any, int, []byte, error) {
